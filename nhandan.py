...
class NewsSpider(scrapy.Spider):
    ...
    name = "nhandan"
    link_directory = "nhandan"
    crawled_history = "history/nhandan-history.txt"
    crawled_pages = []
    ...
    def parse(self, response):
        ...
        """
        container = response.css("div.item-container")[0]

        title = container.css("h3").extract_first().strip()
        title = BeautifulSoup(title, "lxml").text.strip()

        description = container.css("div.sapo p").extract_first().strip()
        description = BeautifulSoup(description, "lxml").text.strip()

        paragraphs = container.css("div.item-content p").extract()
        paragraphs = [BeautifulSoup(p, "lxml").text.strip() for p in paragraphs]
        paragraphs = [x for x in paragraphs if len(x) > 0]

        author = container.css("p.author").extract()[0]
        # print(author)
        author = BeautifulSoup(author, "lxml").text.strip()

        time = container.css('h5.date-created').extract_first()
