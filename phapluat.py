...
class NewsSpider(scrapy.Spider):
    ...
    name = "phapluat"
    crawled_history = "history/phapluat-history.txt"
    crawled_pages = []
    ...
    def parse(self, response):
        ...
        """
        container = response.css("div.main-body")[0]

        title = container.css("h1.main-title.cms-title").extract_first().strip()
        title = BeautifulSoup(title, "lxml").text.strip()

        description = container.css("article.article-body em").extract_first().strip()
        description = BeautifulSoup(description, "lxml").text.strip()

        paragraphs = container.css("article.article-body div#main_detail p").extract()
        paragraphs = [BeautifulSoup(p, "lxml").text.strip() for p in paragraphs]
        paragraphs = [x for x in paragraphs if len(x) > 0]

        author = container.css("div#main_detail strong.cms-author").extract()[0]
