...
class NewsSpider(scrapy.Spider):
    ...
    name = "laodong"
    crawled_history = "history/laodong-history.txt"
    crawled_pages = []
    ...
    def parse(self, response):
        ...
        description = container.css("div.left-sidebar p.abs").extract_first().strip()
        description = BeautifulSoup(description, "lxml").text.strip()

        paragraphs = container.css("div.article-content p").extract()
        paragraphs = [BeautifulSoup(p, "lxml").text.strip() for p in paragraphs]
        paragraphs = [x for x in paragraphs if len(x) > 0]

        author = container.css("div.left-sidebar span.author").extract()[0]
        author = BeautifulSoup(author, "lxml").text.strip()

        time = container.css('div.time time').extract_first()
