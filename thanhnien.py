...
class NewsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        """
        container = response.css("div#storybox")[0]

        title = container.css("h1.details__headline").extract_first().strip()
        title = BeautifulSoup(title, "lxml").text.strip()

        description = container.css("div#chapeau").extract_first()
        description = BeautifulSoup(description, "lxml").text.strip()

        paragraphs = []
        _p = container.css("div#abody").extract_first()
        _p = BeautifulSoup(_p,'lxml')

        for para in _p.children:
            paragraphs.append(para.text.strip())

        author = container.css("div.details__author h4 a").extract()[0]
        author = BeautifulSoup(author, "lxml").text.strip()

        time = container.css('div.details__meta time').extract_first()
